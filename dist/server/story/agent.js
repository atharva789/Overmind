...
export async function checkAndRunStoryAgent(projectRoot) {
    ...
    try {
        ...
        const { rows: unclustered } = await pool.query("SELECT id, content, username, created_at FROM queries WHERE feature_id IS NULL AND project_id = $1 ORDER BY created_at ASC", [projectId]);
        if (unclustered.length > 0) {
            ...
            for (const query of unclustered) {
                ...
                if (res)
                    results.push({ queryId: query.id, ...res });
            }
        }
        return results;
    }
...
export async function generateInitialStory(ai, model, projectRoot, projectId, initialContext = "") {
...
async function evaluateAndClusterQuery(ai, model, projectRoot, projectId, query) {
...
`;
    ...
    try {
        if (decision.action === "assign_existing" && decision.feature_id) {
            ...
            const insertRes = await client.query("INSERT INTO features (project_id, title, description) VALUES ($1, $2, $3) RETURNING id", [projectId, decision.title, decision.description]);
...
export async function regenerateStoryMarkdown(projectRoot, projectId) {
    console.log(`[story-agent] Regenerating story.md for '${projectId}'...`);
    // Fetch all features, oldest first
    const { rows: features } = await pool.query("SELECT id, title, description, created_at FROM features WHERE project_id = $1 ORDER BY created_at ASC", [projectId]);
