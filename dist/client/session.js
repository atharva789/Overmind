...
export class Session {
    ...
    _handleMessage(msg) {
        switch (msg.type) {
            case "join-ack":
                console.log(`[session] Joined party ${msg.payload.partyCode} | Members: ${msg.payload.members.join(", ")} | Host: ${msg.payload.isHost}`);
                break;
            case "member-joined":
            ...
            case "member-left":
                ...
                break;
            case "activity":
                console.log(`[session] Activity — ${msg.payload.username}: ${msg.payload.event}`);
                break;
            case "prompt-queued":
                console.log(`[session] Prompt ${msg.payload.promptId} queued at position ${msg.payload.position}`);
                break;
            case "host-review-request":
                console.log(`[session] [HOST] Review request — ${msg.payload.username} submitted prompt ${msg.payload.promptId}`);
                break;
            case "prompt-approved":
                console.log(`[session] Prompt ${msg.payload.promptId} approved`);
                break;
            case "prompt-denied":
                console.log(`[session] Prompt ${msg.payload.promptId} denied: ${msg.payload.reason}`);
                break;
            case "prompt-greenlit":
                console.log(`[session] Prompt ${msg.payload.promptId} greenlit: ${msg.payload.reasoning}`);
                break;
            case "prompt-redlit":
                console.log(`[session] Prompt ${msg.payload.promptId} redlit: ${msg.payload.reasoning}`);
                break;
            case "error":
