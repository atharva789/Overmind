...
import type { EvaluationResult, GlmEvalRequest } from "./evaluate.js";
...
import {
    GREENLIGHT_BACKEND_DEFAULT,
    LOG_TRUNCATE_CHARS,
    MAX_CONTEXT_PAYLOAD_CHARS,
...
function getBackend(): "glm" | "gemini" {
...
export async function evaluatePrompt(
    ...
    greenlightLog(
        ...
        entry.promptId,
        backend,
        `evaluating (prompt length: ${entry.content.length}, scope: ${entry.scope?.join(",") ?? "none"}, concurrent: ${concurrent.length})`
    );
...
function buildGlmRequest(
    entry: PromptEntry,
    concurrent: PromptEntry[],
    overlapHint: ReturnType<typeof detectScopeOverlap>
): GlmEvalRequest {
...
function readRootContext(): string {
...
function findRelatedContextFiles(
    scope?: string[]
    ...
    for (const s of scope) {
        if (totalChars > MAX_CONTEXT_PAYLOAD_CHARS / 2) break;
...
function gatherCodeSnippets(
    scope?: string[]
): Array<{ path: string; content: string; note?: string }> {
