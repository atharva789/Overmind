...
// ─── Orchestrator ───
export function get_OVERMIND_ORCHESTRATOR_URL(): string { return env("OVERMIND_ORCHESTRATOR_URL"); }
export function get_AGENT_CMD(): string { return env("OVERMIND_AGENT_CMD", "claude"); }
export function get_AGENT_ARGS(): string[] {
    return env("OVERMIND_AGENT_ARGS", "--dangerously-skip-permissions -p").split(" ");
}
export function get_AGENT_TIMEOUT_S(): number { return envNum("OVERMIND_AGENT_TIMEOUT", 300); }
export function get_OVERMIND_ORCHESTRATOR_POLL_MS(): number { return envNum("OVERMIND_ORCHESTRATOR_POLL_MS", 500); }
export function get_OVERMIND_ORCHESTRATOR_TIMEOUT_MS(): number {
    return envNum("OVERMIND_ORCHESTRATOR_TIMEOUT_MS", 15 * MINUTE_MS);
}
export function get_OVERMIND_WRITE_ALLOWLIST(): string[] {
    return env("OVERMIND_WRITE_ALLOWLIST")
        .split(",")
        .map((entry) => entry.trim())
        .filter((entry) => entry.length > 0);
}
export function get_MAX_CONCURRENT_SANDBOXES(): number { return envNum("OVERMIND_MAX_AGENTS", 3); }
...
export function get_ALWAYS_SYNC_PATTERNS(): string[] {
